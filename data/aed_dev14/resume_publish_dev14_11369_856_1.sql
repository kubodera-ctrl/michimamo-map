...
do $guard$
begin
  if (select count(*) from public.safety_spots where facility_type='aed' and active and not duplicate_candidate) <> 45076
...
begin
  if (select count(*) from public.safety_spots p join dev10_batch b using(source_key)
     ...
     or (select count(*) from public.safety_spots where facility_type='aed' and active and not duplicate_candidate) <> 45084
