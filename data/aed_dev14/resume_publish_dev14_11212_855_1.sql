...
do $guard$
begin
  if (select count(*) from public.safety_spots where facility_type='aed' and active and not duplicate_candidate) <> 44065
...
begin
  if (select count(*) from public.safety_spots p join dev10_batch b using(source_key)
     ...
     or (select count(*) from public.safety_spots where facility_type='aed' and active and not duplicate_candidate) <> 44070
