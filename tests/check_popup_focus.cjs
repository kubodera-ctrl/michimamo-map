...
assert.match(html, /const desiredMarkerX = map\.getSize\(\)\.x \/ 2/);
assert.match(html, /safeTop \+ popupRect\.height/);
assert.match(html, /const panX = markerPoint\.x - desiredMarkerX/);
assert.match(html, /const panY = markerPoint\.y - desiredMarkerY/);
