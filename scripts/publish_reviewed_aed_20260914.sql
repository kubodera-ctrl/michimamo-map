...
begin
  ...
  if (select count(*) from public.safety_spots where facility_type='aed' and active and not duplicate_candidate) <> 16051 then
    raise exception 'Production baseline changed; reconcile again before publication';
  end if;
  if (select count(*) from public.safety_spots_nationwide_stage where source_key like 'bodik-reviewed:%') <> 12502 then
    raise exception 'Reviewed staging batch is incomplete or changed';
  end if;
  if exists (
    select 1 from public.safety_spots_nationwide_stage
    where source_key like 'bodik-reviewed:%'
  ...
  where source_key like 'bodik-reviewed:%' and not active and not duplicate_candidate
  on conflict (source_key) do nothing;
  get diagnostics inserted_count = row_count;
  if inserted_count <> 12502 then raise exception 'Unexpected inserted count: %', inserted_count; end if;
