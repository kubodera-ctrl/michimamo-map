"""Build guarded publication SQL for newly recovered dev16 priority AED sources only."""
...
def main() -> None:
    ...
    for item in catalog:
        ...
        if item.get("fetch_status") != "downloaded" or not license_id:
            reports.append({"code": code, "municipality": item.get("municipality"), "status": "not_publishable_source"})
            continue
        records = read_records(Path(item["snapshot"]).read_bytes())
        prefecture = item["prefecture"]
        municipality = item["municipality"]
        key = f"dev16_{code}_{selected['resource_id']}"
        source = {
            "key": key,
            "prefecture": prefecture,
            "municipality": municipality,
            "source_url": item["url"],
            "source_name": municipality + " AED設置情報（自治体公式座標・まちまもMAP dev16審査済み）",
        ...
        if not accepted:
            reports.append({"key": key, "code": code, "municipality": municipality, "raw_rows": len(records), "publish": 0, "excluded": excluded})
            continue
        ...
        source["review_reason"] = "自治体公式オープンデータ・公式座標・利用条件・重複をdev16で確認"
        report = {
            "key": key,
            "code": code,
            "municipality": municipality,
        ...
        for number, start in enumerate(range(0, len(accepted), 150), 1):
            batch = accepted[start:start + 150]
            batch_source = {**source, "key": f"{key}_{number}"}
    ...
    (ROOT / "dev16_priority_review_reports.json").write_text(json.dumps(reports, ensure_ascii=False, indent=2, default=str) + "\n", encoding="utf-8")
    (ROOT / "dev16_priority_batch_index.json").write_text(json.dumps(batches, ensure_ascii=False, indent=2, default=str) + "\n", encoding="utf-8")
    print(json.dumps({"baseline": BASELINE, "batches": batches, "expected_final": baseline}, ensure_ascii=False))
